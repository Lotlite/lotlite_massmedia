          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-6 mt-4 md:mt-0">
              {['Privacy Policy', 'Terms of Service'].map((item) => {
                const href = item === 'Terms of Service' ? '/terms-of-service' :
                            item === 'Privacy Policy' ? '/privacy-policy' : '/';
                return (
                  <Link key={item} href={href} className="relative group text-gray-400 hover:text-white text-sm transition-colors inline-block">
                    {item}
                    <span className="block h-[2px] bg-white w-0 transition-all duration-300 group-hover:w-full"></span>
                  </Link>
                );
